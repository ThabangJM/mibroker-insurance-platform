...
import { ArrowLeft, ArrowRight, User, Phone, Mail, Car, Home as HomeIcon, Shield, Building, CheckCircle, AlertCircle, Eye, FileText, AlertTriangle, Package, Mountain, Calendar, Plus, X } from 'lucide-react';
import { InsuranceType, Representative } from '../types';
import { OPTIONAL_COVER_MAX, OPTIONAL_COVER_MIN, validateAmountRange } from '../utils/coverageValidation';
...
interface NeedsAnalysisData {
  ...
  currentSituation: {
    ...
    claimsHistory: {
      hasClaimsLastFiveYears: boolean;
      claimsList?: Array<{
        date: string;
        type: string;
        amount: string;
      ...
      numberOfClaimsLast12Months?: number;
  ...
  coveragePreferences: {
    ...
    coverageLevel: 'basic' | 'comprehensive' | 'premium' | 'third-party-theft' | 'third-party-only' | 'vehicle-comprehensive' | 'building-contents';
    additionalCoverage: string[];
    additionalCoverRequirements?: string;
    optionalCovers?: {
      accidentalDamage: { selected: boolean; amount: string };
      powerSurge: { selected: boolean; amount: string };
      subsidenceLandslip: { selected: boolean; amount: string };
    };
...
export function QuoteForm({ insuranceType, onSubmit, onBack, loading = false, assignedRepresentative }: QuoteFormProps) {
  ...
  const [formData, setFormData] = useState<FormData & { 
    ...
    needsAnalysis: {
      ...
      coveragePreferences: {
        ...
        additionalCoverage: [],
        optionalCovers: {
          accidentalDamage: { selected: false, amount: '' },
          powerSurge: { selected: false, amount: '' },
          subsidenceLandslip: { selected: false, amount: '' }
        },
  ...
  // Partner ID fields state for dynamic add/remove
  const [partnerIdFields, setPartnerIdFields] = useState<string[]>(['']);
  ...
  const ERROR_DISPLAY_SECONDS = 10;

  // Show error summary with countdown when validation errors appear
  useEffect(() => {
    if (Object.keys(validationErrors).length > 0) {
      setShowErrorSummary(true);
      setErrorCountdown(ERROR_DISPLAY_SECONDS);
  ...
  const dismissErrorSummary = useCallback(() => {
    setShowErrorSummary(false);
    setErrorCountdown(0);
    if (errorTimerRef.current) clearInterval(errorTimerRef.current);
  }, []);
  ...
  const updateFormData = (section: keyof FormData, field: string, value: string | number | boolean | string[] | null) => {
    setFormData(prev => ({
      ...prev,
  ...
  const validatePhone = (phone: string): string | null => {
    ...
    if (!phoneRegex.test(cleaned)) {
      return 'Please enter a valid South African phone number (e.g., 0812345678 or +27812345678)';
    }
    return null;
  };
  ...
  const validateCompanyInfo = (): boolean => {
    ...
    if (formData.companyInfo.businessType === 'private-company') {
    ...
    if (formData.companyInfo.businessType === 'sole-proprietorship') {
      const ownerIdError = validateSAIdNumber(formData.companyInfo.ownerIdNumber || '');
      if (ownerIdError) errors['companyInfo.ownerIdNumber'] = ownerIdError;
    }

    if (formData.companyInfo.businessType === 'partnership') {
      const partnerIdsError = validateRequired(formData.companyInfo.partnerIdNumbers || '', 'Partner ID numbers');
      if (partnerIdsError) errors['companyInfo.partnerIdNumbers'] = partnerIdsError;
    }

    if (formData.companyInfo.businessType === 'npo') {
      const npoNumberError = validateRequired(formData.companyInfo.npoNumber || '', 'NPO number');
      if (npoNumberError) errors['companyInfo.npoNumber'] = npoNumberError;
    }

    if (formData.companyInfo.businessType === 'trust') {
      const trustRegError = validateRequired(formData.companyInfo.trustRegistrationNumber || '', 'Trust registration number');
      if (trustRegError) errors['companyInfo.trustRegistrationNumber'] = trustRegError;
    }
  ...
  const validateCurrentSituation = (): boolean => {
    ...
    // If has claims, validate claims list and number of claims in last 12 months
    if (formData.needsAnalysis.currentSituation.claimsHistory.hasClaimsLastFiveYears) {
      const claimsList = formData.needsAnalysis.currentSituation.claimsHistory.claimsList || [];
      if (claimsList.length === 0) {
        errors['needsAnalysis.currentSituation.claimsHistory.claimsList'] = 'Please add at least one claim';
      } else {
        // Validate each claim in the list
        claimsList.forEach((claim, index) => {
          if (!claim.date) {
            errors[`needsAnalysis.currentSituation.claimsHistory.claimsList.${index}.date`] = 'Claim date is required';
          }
          if (!claim.type) {
            errors[`needsAnalysis.currentSituation.claimsHistory.claimsList.${index}.type`] = 'Claim type is required';
          }
          if (!claim.amount) {
            errors[`needsAnalysis.currentSituation.claimsHistory.claimsList.${index}.amount`] = 'Claim amount is required';
          }
          if (!claim.description?.trim()) {
            errors[`needsAnalysis.currentSituation.claimsHistory.claimsList.${index}.description`] = 'Claim description is required';
          }
        });
      }
      if (!formData.needsAnalysis.currentSituation.claimsHistory.numberOfClaimsLast12Months && formData.needsAnalysis.currentSituation.claimsHistory.numberOfClaimsLast12Months !== 0) {
        errors['needsAnalysis.currentSituation.claimsHistory.numberOfClaimsLast12Months'] = 'Number of claims in last 12 months is required';
      }
    }
    ...
    if (!formData.needsAnalysis.currentSituation.claimsHistory.hasClaimsLastFiveYears) {
      setValidationErrors(prev => {
        ...
        delete newErrors['needsAnalysis.currentSituation.claimsHistory.claimsList'];
        delete newErrors['needsAnalysis.currentSituation.claimsHistory.numberOfClaimsLast12Months'];
        // Clear individual claim errors
        Object.keys(newErrors).forEach(key => {
          if (key.includes('claimsList')) {
  ...
  const validateCoverageNeeds = (): boolean => {
    const errors: {[key: string]: string} = {};
    const optionalCovers = formData.needsAnalysis.coveragePreferences.optionalCovers;
    const validateOptionalCoverAmount = (
      amount: string,
      label: string,
      errorKey: string
    ) => {
      const amountError = validateAmountRange(amount || '', label, OPTIONAL_COVER_MIN, OPTIONAL_COVER_MAX);
      if (amountError) {
        errors[errorKey] = amountError;
      }
    };
    ...
    if (optionalCovers?.accidentalDamage?.selected) {
      validateOptionalCoverAmount(
        optionalCovers.accidentalDamage.amount,
        'Accidental Damage amount',
        'needsAnalysis.coveragePreferences.optionalCovers.accidentalDamage.amount'
      );
    }

    if (optionalCovers?.powerSurge?.selected) {
      validateOptionalCoverAmount(
        optionalCovers.powerSurge.amount,
        'Power Surge & Electrical Breakdown amount',
        'needsAnalysis.coveragePreferences.optionalCovers.powerSurge.amount'
      );
    }

    if (insuranceType === 'buildings-insurance' && optionalCovers?.subsidenceLandslip?.selected) {
      validateOptionalCoverAmount(
        optionalCovers.subsidenceLandslip.amount,
        'Subsidence and Landslip amount',
        'needsAnalysis.coveragePreferences.optionalCovers.subsidenceLandslip.amount'
      );
    }
    ...
    <div className="space-y-6">
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className={`pl-12 w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${
      ...
      <div className="bg-green-50 p-6 rounded-xl space-y-4 border-2 border-green-200">
        <div className="flex items-center justify-between mb-4">
          <h4 className="text-lg font-semibold text-gray-900 mb-4">Co-Insured Information (Optional)</h4>
          ...
          <div className="space-y-6">
            ...
            {!formData.coInsured?.sameAddress && (
              <div className="bg-yellow-50 p-4 rounded-lg space-y-4 border-2 border-yellow-200">
                <h5 className="text-lg font-semibold text-gray-900 mb-4">Co-Insured Physical Address</h5>
  ...
  const renderCompanyInfo = () => (
    <div className="space-y-6">
      {/* Company Name and Business Type Row */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">
            Company Name *
      ...
      {formData.companyInfo.businessType === 'private-company' && (
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">
            Registration Number *
          </label>
      ...
      {formData.companyInfo.businessType === 'sole-proprietorship' && (
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">
            Owner ID Number *
          </label>
          <div className="relative">
            ...
            <input
              type="text"
              value={formData.companyInfo.ownerIdNumber || ''}
              onChange={(e) => updateFormData('companyInfo', 'ownerIdNumber', e.target.value)}
              className={`pl-12 w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${
                hasFieldError('companyInfo.ownerIdNumber') 
                  ? 'border-red-300 bg-red-50' 
                  : 'border-gray-300 focus:border-blue-500'
              }`}
          ...
          {hasFieldError('companyInfo.ownerIdNumber') && (
            <p className="mt-1 text-sm text-red-600 flex items-center">
              <AlertCircle className="w-4 h-4 mr-1" />
              {getFieldError('companyInfo.ownerIdNumber')}
            </p>
          )}
        </div>
      )}

      {formData.companyInfo.businessType === 'partnership' && (
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">
            Partner ID Numbers *
          </label>
          <div className="space-y-3">
            {partnerIdFields.map((id, index) => (
              <div key={index} className="flex items-center gap-2">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    value={id}
                    onChange={(e) => {
                      const newFields = [...partnerIdFields];
                      newFields[index] = e.target.value;
                      setPartnerIdFields(newFields);
                      updateFormData('companyInfo', 'partnerIdNumbers', newFields.filter(v => v.trim() !== '').join(', '));
                    }}
                    className={`pl-12 w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${
                      hasFieldError('companyInfo.partnerIdNumbers') 
                        ? 'border-red-300 bg-red-50' 
                        : 'border-gray-300 focus:border-blue-500'
                    }`}
                    placeholder={`Enter partner ${index + 1} ID number`}
                  />
                </div>
                {partnerIdFields.length > 1 && (
                  <button
                    type="button"
                    onClick={() => {
                      const newFields = partnerIdFields.filter((_, i) => i !== index);
                      setPartnerIdFields(newFields);
                      updateFormData('companyInfo', 'partnerIdNumbers', newFields.filter(v => v.trim() !== '').join(', '));
                    }}
            ...
            <button
              type="button"
              onClick={() => {
                setPartnerIdFields([...partnerIdFields, '']);
              }}
              className="flex items-center gap-2 px-4 py-2 text-blue-600 hover:text-blue-800 hover:bg-blue-50 rounded-lg font-medium transition-colors duration-200"
            >
          ...
          {hasFieldError('companyInfo.partnerIdNumbers') && (
            <p className="mt-1 text-sm text-red-600 flex items-center">
              <AlertCircle className="w-4 h-4 mr-1" />
              {getFieldError('companyInfo.partnerIdNumbers')}
            </p>
          )}
        </div>
      )}

      {formData.companyInfo.businessType === 'npo' && (
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">
            NPO Number *
          </label>
          <div className="relative">
            ...
            <input
              type="text"
              value={formData.companyInfo.npoNumber || ''}
              onChange={(e) => updateFormData('companyInfo', 'npoNumber', e.target.value)}
              className={`pl-12 w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${
                hasFieldError('companyInfo.npoNumber') 
                  ? 'border-red-300 bg-red-50' 
                  : 'border-gray-300 focus:border-blue-500'
              }`}
          ...
          {hasFieldError('companyInfo.npoNumber') && (
            <p className="mt-1 text-sm text-red-600 flex items-center">
              <AlertCircle className="w-4 h-4 mr-1" />
              {getFieldError('companyInfo.npoNumber')}
            </p>
          )}
        </div>
      )}

      {formData.companyInfo.businessType === 'trust' && (
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">
          ...
          <div className="relative">
            ...
            <input
              type="text"
              value={formData.companyInfo.trustRegistrationNumber || ''}
              onChange={(e) => updateFormData('companyInfo', 'trustRegistrationNumber', e.target.value)}
              className={`pl-12 w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${
                hasFieldError('companyInfo.trustRegistrationNumber') 
                  ? 'border-red-300 bg-red-50' 
                  : 'border-gray-300 focus:border-blue-500'
              }`}
          ...
          {hasFieldError('companyInfo.trustRegistrationNumber') && (
            <p className="mt-1 text-sm text-red-600 flex items-center">
              <AlertCircle className="w-4 h-4 mr-1" />
              {getFieldError('companyInfo.trustRegistrationNumber')}
            </p>
          )}
        </div>
      )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <p className="mt-2 text-xs text-gray-500">
              If you see a validation error, confirm the number is 10 digits (starts with 0) or uses +27.
            </p>
            {hasFieldError('companyInfo.phone') && (
              <p className="mt-1 text-sm text-red-600 flex items-center">
                <AlertCircle className="w-4 h-4 mr-1" />
        ...
        <div className="bg-yellow-50 p-4 rounded-lg">
          ...
          <div className="space-y-2">
            <label className="flex items-center">
              <input
                type="radio"
                name="hasClaimsLastFiveYears"
                value="true"
                checked={formData.needsAnalysis.currentSituation.claimsHistory.hasClaimsLastFiveYears === true}
                onChange={(e) => setFormData(prev => ({
                  ...prev,
                  needsAnalysis: {
                    ...prev.needsAnalysis,
                    currentSituation: {
                      ...
                      claimsHistory: {
                        ...
                        hasClaimsLastFiveYears: e.target.value === 'true',
                        claimsList: e.target.value === 'true' ? [{ date: '', type: '', amount: '', description: '' }] : []
                      }
                    }
                  }
                }))}
            ...
            <label className="flex items-center">
              <input
                ...
                name="hasClaimsLastFiveYears"
                value="false"
                checked={formData.needsAnalysis.currentSituation.claimsHistory.hasClaimsLastFiveYears === false}
                onChange={(e) => setFormData(prev => ({
                  ...prev,
                  needsAnalysis: {
                    ...prev.needsAnalysis,
                    currentSituation: {
                      ...
                      claimsHistory: {
                        hasClaimsLastFiveYears: e.target.value === 'true',
                        claimsList: [],
                        numberOfClaimsLast12Months: undefined,
          ...
          {formData.needsAnalysis.currentSituation.claimsHistory.hasClaimsLastFiveYears && (
            <div className="mt-4 space-y-4 p-4 bg-orange-50 border border-orange-200 rounded-lg">
              <h4 className="text-sm font-medium text-gray-800 mb-3">Claims History (Last 5 Years)</h4>
              
              {/* Editable Claims List */}
              <div className="space-y-4">
                {(formData.needsAnalysis.currentSituation.claimsHistory.claimsList || []).map((claim, index) => (
                  <div key={index} className="bg-white p-4 rounded-lg border border-orange-300 relative">
                    <div className="flex justify-between items-start mb-3">
                      <h5 className="font-medium text-gray-800">Claim {index + 1}</h5>
                      {(formData.needsAnalysis.currentSituation.claimsHistory.claimsList || []).length > 1 && (
                        <button
                          type="button"
                          onClick={() => {
                            const newClaimsList = [...(formData.needsAnalysis.currentSituation.claimsHistory.claimsList || [])];
                            newClaimsList.splice(index, 1);
                            setFormData(prev => ({
                              ...prev,
                              needsAnalysis: {
                                ...prev.needsAnalysis,
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            const newClaimsList = [...(formData.needsAnalysis.currentSituation.claimsHistory.claimsList || [])];
                            newClaimsList[index].date = e.target.value;
                            setFormData(prev => ({
                              ...prev,
                              needsAnalysis: {
                                ...prev.needsAnalysis,
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) => {
                            const newClaimsList = [...(formData.needsAnalysis.currentSituation.claimsHistory.claimsList || [])];
                            newClaimsList[index].type = e.target.value;
                            setFormData(prev => ({
                              ...prev,
                              needsAnalysis: {
                                ...prev.needsAnalysis,
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            const newClaimsList = [...(formData.needsAnalysis.currentSituation.claimsHistory.claimsList || [])];
                            newClaimsList[index].amount = e.target.value;
                            setFormData(prev => ({
                              ...prev,
                              needsAnalysis: {
                                ...prev.needsAnalysis,
                      ...
                      <div className="md:col-span-2">
                        ...
                        <textarea
                          ...
                          onChange={(e) => {
                            const newClaimsList = [...(formData.needsAnalysis.currentSituation.claimsHistory.claimsList || [])];
                            newClaimsList[index].description = e.target.value;
                            setFormData(prev => ({
                              ...prev,
                              needsAnalysis: {
                                ...prev.needsAnalysis,
                ...
                <button
                  ...
                  onClick={() => {
                    const newClaimsList = [
                      ...(formData.needsAnalysis.currentSituation.claimsHistory.claimsList || []),
                  ...
                  className="w-full p-3 border-2 border-dashed border-blue-300 rounded-lg text-blue-600 hover:bg-blue-50 hover:border-blue-400 transition-colors"
                >
                  + Add Another Claim
                </button>
              </div>
              ...
              <div className="mt-6 pt-6 border-t border-orange-300">
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  How many of these claims occurred in the last 12 months? *
                </label>
                <select
                  value={formData.needsAnalysis.currentSituation.claimsHistory.numberOfClaimsLast12Months || ''}
                  onChange={(e) => setFormData(prev => ({
                    ...prev,
                    needsAnalysis: {
                      ...prev.needsAnalysis,
                      currentSituation: {
                        ...
                        claimsHistory: {
                          ...prev.needsAnalysis.currentSituation.claimsHistory,
                          numberOfClaimsLast12Months: parseInt(e.target.value)
                        }
                      }
                    }
                  }))}
                  className={`w-full p-3 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${
                    validationErrors['needsAnalysis.currentSituation.claimsHistory.numberOfClaimsLast12Months'] 
                      ? 'border-red-300 bg-red-50' 
                      : 'border-gray-300'
                  }`}
                >
                ...
                {/* Warning for multiple claims in last 12 months */}
                {formData.needsAnalysis.currentSituation.claimsHistory.numberOfClaimsLast12Months && 
                 formData.needsAnalysis.currentSituation.claimsHistory.numberOfClaimsLast12Months >= 2 && (
      ...
      <div className="space-y-4">
              ...
              <>
                <label className="flex items-center">
                  <input
                    ...
                    checked={formData.needsAnalysis.coveragePreferences.coverageLevel === 'vehicle-comprehensive'}
                    onChange={(e) => setFormData(prev => ({
                      ...prev,
                      needsAnalysis: {
                        ...prev.needsAnalysis,
                    ...
                    className="mr-2"
                  />
                  <span>Vehicle Comprehensive Cover</span>
                </label>
                <label className="flex items-center">
                  <input
                    type="radio"
                    ...
                    checked={formData.needsAnalysis.coveragePreferences.coverageLevel === 'building-contents'}
                    onChange={(e) => setFormData(prev => ({
                      ...prev,
                      needsAnalysis: {
                        ...prev.needsAnalysis,
                    ...
                    className="mr-2"
                  />
                  <span>Building and Contents Cover</span>
                </label>
              </>
            ) : (
              <>
        ...
        {['buildings-insurance', 'household-contents'].includes(insuranceType) && (
          <div className="bg-blue-50 p-4 rounded-lg space-y-4">
            <div>
              <h4 className="text-lg font-semibold text-gray-900 mb-4">Optional Covers</h4>
              <p className="text-sm text-gray-600 mt-1">
                Select any optional covers and enter the specific cover amount (R10,000 - R100,000).
              </p>
            </div>
            ...
            <div className="space-y-3">
              <div className="flex items-start justify-between gap-4">
                <label className="flex items-center">
                  <input
                    type="checkbox"
                    checked={formData.needsAnalysis.coveragePreferences.optionalCovers?.accidentalDamage.selected || false}
                    onChange={(e) => {
                      ...
                      setFormData(prev => ({
                        ...
                        needsAnalysis: {
                          ...
                          coveragePreferences: {
                            ...prev.needsAnalysis.coveragePreferences,
                            optionalCovers: {
                              ...prev.needsAnalysis.coveragePreferences.optionalCovers,
                              accidentalDamage: {
                                selected: isChecked,
                                amount: isChecked
                                  ? prev.needsAnalysis.coveragePreferences.optionalCovers?.accidentalDamage.amount || ''
                                  : ''
                              }
                            }
                          }
                ...
                <div className="w-48">
                  <div className="relative">
                    ...
                    <input
                      ...
                      max="100000"
                      step="1000"
                      disabled={!formData.needsAnalysis.coveragePreferences.optionalCovers?.accidentalDamage.selected}
                      value={formData.needsAnalysis.coveragePreferences.optionalCovers?.accidentalDamage.amount || ''}
                      onChange={(e) => setFormData(prev => ({
                        ...prev,
                        needsAnalysis: {
                          ...prev.needsAnalysis,
                          coveragePreferences: {
                            ...
                            optionalCovers: {
                              ...prev.needsAnalysis.coveragePreferences.optionalCovers,
                              accidentalDamage: {
                                selected: prev.needsAnalysis.coveragePreferences.optionalCovers?.accidentalDamage.selected || false,
                      ...
                      className={`w-full pl-8 pr-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${
                        validationErrors['needsAnalysis.coveragePreferences.optionalCovers.accidentalDamage.amount']
                          ? 'border-red-300 bg-red-50'
                          : 'border-gray-300'
                      }`}
                      placeholder="10000"
              ...
              <div className="flex items-start justify-between gap-4">
                <label className="flex items-center">
                  <input
                    type="checkbox"
                    checked={formData.needsAnalysis.coveragePreferences.optionalCovers?.powerSurge.selected || false}
                    onChange={(e) => {
                      const isChecked = e.target.checked;
                      setFormData(prev => ({
                        ...prev,
                          ...
                          coveragePreferences: {
                            ...
                            optionalCovers: {
                              ...
                              powerSurge: {
                                selected: isChecked,
                                amount: isChecked
                                  ? prev.needsAnalysis.coveragePreferences.optionalCovers?.powerSurge.amount || ''
                                  : ''
                              }
                            }
                          }
                    ...
                    className="mr-2"
                  />
                  <span>Power Surge & Electrical Breakdown</span>
                </label>
                <div className="w-48">
                  <div className="relative">
                    <span className="absolute left-3 top-2.5 text-gray-500">R</span>
                    <input
                      ...
                      max="100000"
                      step="1000"
                      disabled={!formData.needsAnalysis.coveragePreferences.optionalCovers?.powerSurge.selected}
                      value={formData.needsAnalysis.coveragePreferences.optionalCovers?.powerSurge.amount || ''}
                      onChange={(e) => setFormData(prev => ({
                        ...prev,
                        needsAnalysis: {
                          ...prev.needsAnalysis,
                          coveragePreferences: {
                            ...
                            optionalCovers: {
                              ...prev.needsAnalysis.coveragePreferences.optionalCovers,
                              powerSurge: {
                                selected: prev.needsAnalysis.coveragePreferences.optionalCovers?.powerSurge.selected || false,
                                amount: e.target.value
                              }
                            }
                          }
                      ...
                      className={`w-full pl-8 pr-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${
                        validationErrors['needsAnalysis.coveragePreferences.optionalCovers.powerSurge.amount']
                          ? 'border-red-300 bg-red-50'
                          : 'border-gray-300'
                      }`}
                      placeholder="10000"
              ...
              {insuranceType === 'buildings-insurance' && (
                <div className="flex items-start justify-between gap-4">
                  <label className="flex items-center">
                    <input
                      type="checkbox"
                      checked={formData.needsAnalysis.coveragePreferences.optionalCovers?.subsidenceLandslip.selected || false}
                      onChange={(e) => {
                        const isChecked = e.target.checked;
                        setFormData(prev => ({
                          ...prev,
                            ...
                            coveragePreferences: {
                              ...
                              optionalCovers: {
                                ...
                                subsidenceLandslip: {
                                  selected: isChecked,
                                  amount: isChecked
                                    ? prev.needsAnalysis.coveragePreferences.optionalCovers?.subsidenceLandslip.amount || ''
                                    : ''
                                }
                              }
                            }
                      ...
                      className="mr-2"
                    />
                    <span>Subsidence and Landslip Full Cover</span>
                  </label>
                  <div className="w-48">
                    <div className="relative">
                      <span className="absolute left-3 top-2.5 text-gray-500">R</span>
                      <input
                        ...
                        max="100000"
                        step="1000"
                        disabled={!formData.needsAnalysis.coveragePreferences.optionalCovers?.subsidenceLandslip.selected}
                        value={formData.needsAnalysis.coveragePreferences.optionalCovers?.subsidenceLandslip.amount || ''}
                        onChange={(e) => setFormData(prev => ({
                          ...prev,
                          needsAnalysis: {
                            ...prev.needsAnalysis,
                            coveragePreferences: {
                              ...
                              optionalCovers: {
                                ...prev.needsAnalysis.coveragePreferences.optionalCovers,
                                subsidenceLandslip: {
                                  selected: prev.needsAnalysis.coveragePreferences.optionalCovers?.subsidenceLandslip.selected || false,
                                  amount: e.target.value
                                }
                              }
                            }
                        ...
                        className={`w-full pl-8 pr-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${
                          validationErrors['needsAnalysis.coveragePreferences.optionalCovers.subsidenceLandslip.amount']
                            ? 'border-red-300 bg-red-50'
                            : 'border-gray-300'
                        }`}
                        placeholder="10000"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Agent Comment (if limited cover applies)
              </label>
              <textarea
                value={formData.needsAnalysis.coveragePreferences.optionalCoverAgentComment || ''}
                onChange={(e) => setFormData(prev => ({
                  ...prev,
                  needsAnalysis: {
                    ...prev.needsAnalysis,
                    coveragePreferences: {
                      ...prev.needsAnalysis.coveragePreferences,
                      optionalCoverAgentComment: e.target.value
                    }
                  }
                }))}
                rows={3}
                className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                placeholder="Note any insurer limitations or special conditions"
              />
              <p className="text-xs text-gray-500 mt-1">
                Note: Some insurers might offer limited cover; add a comment if applicable.
              </p>
            </div>
          </div>
        )}
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              {/* Dropdown for vehicle value ranges in ZAR */}
              <select
              ...
              >
                ...
                {/* South African Rand value ranges */}
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-6">
          ...
          <div className="space-y-2">
            <label className="flex items-center">
              <input
                ...
                onChange={(e) => {
                  ...
                  updateFormData('insuranceInfo', 'propertySameAsResidential', isSame);
            ...
            <label className="flex items-center">
              <input
                ...
                onChange={(e) => {
                  updateFormData('insuranceInfo', 'propertySameAsResidential', e.target.value === 'true');
                }}
                className="mr-2"
              />
        ...
        {formData.insuranceInfo.propertySameAsResidential === false && (
          <div className="bg-blue-50 p-4 rounded-lg space-y-4">
            <h4 className="text-lg font-semibold text-gray-900 mb-4">Property Physical Address *</h4>
        ...
        <div className="bg-gray-50 p-4 rounded-lg space-y-4">
          <h4 className="text-lg font-semibold text-gray-900 mb-4">Building Construction Details</h4>
        ...
        {(formData.insuranceInfo.buildingUse === 'commercial' || formData.insuranceInfo.buildingUse === 'mixed') && (
          <div className="bg-yellow-50 p-4 rounded-lg space-y-4">
            <h4 className="text-lg font-semibold text-gray-900 mb-4">Commercial Use Details</h4>
      ...
      <div className="space-y-6">
        ...
        {(insuranceType === 'public-liability' || insuranceType === 'small-business' || insuranceType === 'commercial-property') ? (
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              ...
              {formData.companyInfo.ownerIdNumber && (
                <div><strong>Owner ID Number:</strong> {formData.companyInfo.ownerIdNumber}</div>
              )}
              {formData.companyInfo.partnerIdNumbers && (
                <div><strong>Partner ID Numbers:</strong> {formData.companyInfo.partnerIdNumbers}</div>
              )}
              {formData.companyInfo.npoNumber && (
                <div><strong>NPO Number:</strong> {formData.companyInfo.npoNumber}</div>
              )}
              {formData.companyInfo.trustRegistrationNumber && (
                <div><strong>Trust Registration Number:</strong> {formData.companyInfo.trustRegistrationNumber}</div>
              )}
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div><strong>Claims History:</strong> {formData.needsAnalysis.currentSituation.claimsHistory.hasClaimsLastFiveYears ? 'Yes' : 'No claims in last 5 years'}</div>
            {formData.needsAnalysis.currentSituation.claimsHistory.hasClaimsLastFiveYears && (
              <div className="ml-4 space-y-1 text-gray-600">
                <div>• Total claims in last 5 years: {formData.needsAnalysis.currentSituation.claimsHistory.claimsList?.length || 0}</div>
                <div>• Claims in last 12 months: {
                  formData.needsAnalysis.currentSituation.claimsHistory.numberOfClaimsLast12Months === 0 ? 'None' :
                  formData.needsAnalysis.currentSituation.claimsHistory.numberOfClaimsLast12Months === 1 ? 'One' :
                  formData.needsAnalysis.currentSituation.claimsHistory.numberOfClaimsLast12Months === 2 ? 'Two' :
                  formData.needsAnalysis.currentSituation.claimsHistory.numberOfClaimsLast12Months === 3 ? 'More than two' :
                  formData.needsAnalysis.currentSituation.claimsHistory.numberOfClaimsLast12Months
                }</div>
                {formData.needsAnalysis.currentSituation.claimsHistory.claimsList && formData.needsAnalysis.currentSituation.claimsHistory.claimsList.length > 0 && (
                  <div>
                    <div className="font-medium mt-2">Claim Details:</div>
                    {formData.needsAnalysis.currentSituation.claimsHistory.claimsList.map((claim, index) => (
                      <div key={index} className="ml-2 mt-1 p-2 bg-gray-100 rounded">
                        <div>• Claim {index + 1}: {claim.type} - R{claim.amount}</div>
                        <div className="text-xs">  Date: {claim.date}</div>
                        <div className="text-xs">  Description: {claim.description}</div>
                      </div>
                    ))}
                  </div>
                )}
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          <div className="flex justify-between items-center mb-4">
            <h4 className="text-lg font-semibold text-gray-900 mb-4">Coverage Needs</h4>
            <button
          ...
          <div className="space-y-2 text-sm">
            ...
            {['buildings-insurance', 'household-contents'].includes(insuranceType) && (
              <div>
                <strong>Optional Covers:</strong>{' '}
                {(() => {
                  const optionalCovers = formData.needsAnalysis.coveragePreferences.optionalCovers;
                  if (!optionalCovers) return 'None selected';
                  const selections = [
                    optionalCovers.accidentalDamage.selected
                      ? `Accidental Damage (R${optionalCovers.accidentalDamage.amount || '0'})`
                      : null,
                    optionalCovers.powerSurge.selected
                      ? `Power Surge & Electrical Breakdown (R${optionalCovers.powerSurge.amount || '0'})`
                      : null,
                    insuranceType === 'buildings-insurance' && optionalCovers.subsidenceLandslip.selected
                      ? `Subsidence and Landslip Full Cover (R${optionalCovers.subsidenceLandslip.amount || '0'})`
                      : null,
            ...
            {formData.needsAnalysis.coveragePreferences.optionalCoverAgentComment && (
              <div><strong>Agent Comment:</strong> {formData.needsAnalysis.coveragePreferences.optionalCoverAgentComment}</div>
            )}
        ...
        {insuranceType === 'commercial-property' && (
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            <div className="flex justify-between items-center mb-4">
              <h4 className="text-lg font-semibold text-gray-900 mb-4">Property Usage Details</h4>
              <button
        ...
        {['buildings-insurance', 'household-contents'].includes(insuranceType) && formData.needsAnalysis.riskFactors && Object.keys(formData.needsAnalysis.riskFactors).length > 0 && (
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            <div className="flex justify-between items-center mb-4">
              <h4 className="text-lg font-semibold text-gray-900 mb-4">Risk Assessment & Security</h4>
              <button
        ...
        {insuranceType === 'small-business' && (
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            <div className="flex justify-between items-center mb-4">
              <h4 className="text-lg font-semibold text-gray-900 mb-4">Business Assets & Equipment</h4>
              <button
        ...
        {insuranceType === 'transport-insurance' && (
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            <div className="flex justify-between items-center mb-4">
              <h4 className="text-lg font-semibold text-gray-900 mb-4">Fleet Information</h4>
              <button
        ...
        {insuranceType === 'e-hailing' && (
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            <div className="flex justify-between items-center mb-4">
              <h4 className="text-lg font-semibold text-gray-900 mb-4">E-Hailing Service Details</h4>
              <button
        ...
        {insuranceType === 'transport-insurance' && (
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            <div className="flex justify-between items-center mb-4">
              <h4 className="text-lg font-semibold text-gray-900 mb-4">Transport Operations</h4>
              <button
        ...
        {insuranceType === 'engineering-construction' && (formData.projectName || formData.contractValue) && (
          <>
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-6">
              <div className="flex justify-between items-center mb-4">
                <h4 className="text-lg font-semibold text-gray-900 mb-4">Construction Details</h4>
                <button
        ...
        {insuranceType === 'body-corporates' && (
          <>
            <div className="bg-white border border-gray-200 rounded-lg p-6">
              <div className="flex justify-between items-center mb-4">
                <h4 className="text-lg font-semibold text-gray-900 mb-4">Scheme Details</h4>
                <button
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-6">
              <div className="flex justify-between items-center mb-4">
                <h4 className="text-lg font-semibold text-gray-900 mb-4">Common Areas & Facilities</h4>
                <button
        ...
        {insuranceType === 'aviation-marine' && (
          <>
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-6">
              <div className="flex justify-between items-center mb-4">
                <h4 className="text-lg font-semibold text-gray-900 mb-4">Operations Details</h4>
                <button
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100">
          ...
          {/* Validation Summary with Countdown Timer */}
          {showErrorSummary && Object.keys(validationErrors).length > 0 && (
            <div className="px-8 pb-4 transition-opacity duration-500" style={{ opacity: errorCountdown <= 2 ? 0.6 : 1 }}>
              <div className="bg-red-50 border border-red-200 rounded-lg p-4 relative overflow-hidden">
                ...
                <div
                  ...
                  style={{ width: `${(errorCountdown / ERROR_DISPLAY_SECONDS) * 100}%` }}
                />
                <div className="flex items-start">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-xs text-red-400 font-medium">{errorCountdown}s</span>
                        <button
                          onClick={dismissErrorSummary}
